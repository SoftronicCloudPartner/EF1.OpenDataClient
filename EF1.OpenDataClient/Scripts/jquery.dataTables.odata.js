...
function fnServerOData(sUrl, aoData, fnCallback, oSettings) {
    ...
    if (oSettings.oFeatures.bServerSide) {
            ...
            function (i, value) {
                ...
                if ((oParams.sSearch !== null && oParams.sSearch !== "" || columnFilter !== null && columnFilter !== "") && value.bSearchable) {
                    ...
                    case 'html':
                        ...
                        {
                            // enable child object search using any
                            if (value.sChild != null) {
                                asFilters.push(value.sChild + "/any(i: indexof(tolower(i/" + sFieldName + "), '" + oParams.sSearch.toLowerCase() + "') gt -1)");
                            } else {
        ...
        var asOrderBy = [];
        for (
            var i = 0; i < oParams.iSortingCols; i++) {
            if (oParams["iSortCol_" + i]!=0){
